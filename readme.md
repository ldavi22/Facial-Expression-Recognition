...
**შედეგი:** Train სიზუსტე ~65%-ს მიაღწია. Val სიზუსტე 58%-ზე ავიდა.
 
**LR-ის ქცევა:** შედულერი **~6-ჯერ** ამოქმედდა 80 იტერაციის განმავლობაში , LR 1e-3-დან ~1e-5-მდე ჩამოიყვანა.
...
| Weight decay | 1e-4 |
| Batch ზომა | 256 |
| შედულერი | `ReduceLROnPlateau(mode='min', factor=0.7, patience=7, min_lr=1e-5)` |
...
| Weight decay | 1e-4 |
| ბაჩის ზომა | 256 |
| შედულერი | `ReduceLROnPlateau(mode='min', factor=0.5, patience=5, min_lr=1e-5)` |
...
| Weight decay | 1e-4 |
| ბაჩის ზომა | 256 |
| შედულერი | `ReduceLROnPlateau(factor=0.5, patience=5, min_lr=1e-5)` |
| აუგმენტაცია | Flip + Rotation + Translate (GPU-ზე) |
| ეპოქები | 100 |
 
**შედეგი:** val/acc **60%**, train/acc ~74%, train/val სხვაობა ~14%. Val/loss ოსცილირებდა ვიწრო ზოლში ~40-ე ეპოქიდან -- შედულერის ხშირი ამოქმედება ოპტიმიზაციას არესტაბილიზირებდა.
...
### VGG გაშვება 2 — რბილი შედულერი (დაბინძურებული გაშვება)
 
| პარამეტრი | მნიშვნელობა |
|---|---|
| შედულერი | `ReduceLROnPlateau(factor=0.7, patience=7, min_lr=1e-5)` |
| აუგმენტაცია | VGG გაშვება 1-ის იგივე |
| ეპოქები | 150 |
 
**შედეგი:** `~val/acc` **61%**, train/acc ~66%, train/val სხვაობა ~5%.
 
**დიაგნოზი -- დაბინძურებული, არა სუფთა შედარება.** მოდელი გაშვებამდე ხელახლა არ ინიციალიზდა -- VGG გაშვება 1-ის საბოლოო წონებიდან გააგრძელა ტრენინგი. ეს ხსნის გაფართოებულ სხვაობას (მოდელი 100 წინა ეპოქის შემდეგ უკვე ტრენინგ სეტის მიმართ ცრომულობით ჩამოვიდა). 68% ეს კონფიგურაცია სუფთა სახით არ ზომავს.
 
---
 
### VGG გაშვება 3 — LR=3e-3 (ძალიან მაღალი, ადრე შეჩერდა)
 
| პარამეტრი | მნიშვნელობა |
|---|---|
| სწავლის სიჩქარე | 3e-3 |
| შედულერი | `ReduceLROnPlateau(factor=0.5, patience=5)` |
| ეპოქები | ~35-ზე შეჩერდა |
 
**შედეგი:** train/acc ~32% 35-ე ეპოქაზე, val/acc ძლიერ ხმაურიანი (26-36%), გაშვება შეჩერდა.
 
**დიაგნოზი -- LR ძალიან მაღალი.** Train/loss ~2.6-დან დაიწყო (მოსალოდნელ ~1.94-ზე გაცილებით მაღლა). Adam-ის ოპტიმალური ზონა CNN-ებისთვის ჩვეულებრივ 1e-3-დან 3e-4-მდეა; 3e-3 სტაბილური ზონის გარეთაა.
...
### VGG4 გაშვება 1 — გაფართოებული აუგმენტაცია (საუკეთესო შედეგი)
...
**შედეგი:** val/acc **~70%**, train/acc ~73%, train/val სხვაობა ~3%. შედულერი ერთხელ ამოქმედდა (~90-ე ეპოქაზე) LR 1e-3-დან ~7e-4-მდე ჩამოყავდა.
 
**რატომ ColorJitter:** FER2013-ის სურათები ვებ-სკრეპინგითაა მოპოვებული სხვადასხვა განათებით -- სიკაშკაშის/კონტრასტის ვარიაციებისადმი ინვარიანტობა პირდაპირ ამ ცნობილ პრობლემას ებრძვის.
 
---
 
### VGG4 გაშვება 2 — გამაგრებული რეგულარიზაცია (გადარეგულირება)
...
**შედეგი:** val/acc **~60%**, train/val სხვაობა 10%-მდე გაფართოვდა.
 
**დიაგნოზი -- გადარეგულირება, მოდელი ახლა underfitting-ს განიცდის.** VGG4 გაშვება 1-ს მხოლოდ 3%-იანი სხვაობა ჰქონდა -- პრაქტიკულად ოვერფიტი არ იყო. გამაგრებული რეგულარიზაცია გაცილებით მეტი იყო ვიდრე საჭირო. გაკვეთილი: რეგულარიზაციის ცვლილებები მხოლოდ მაშინ არის გამართლებული, როცა მკაფიო, ფართო, მდგრადი train/val სხვაობა არსებობს. 3%-იანი სხვაობა ამ კრიტერიუმს არ აკმაყოფილებს.
 
---
 
## ResNet-სტილის ექსპერიმენტები (მე-3 ნოუთბუქი)
 
### მოტივაცია
 
VGG4StyleNet-მა ~70% val სიზუსტეზე მიაღწია პლატოს. შემდეგი არქიტექტურული ნაბიჯი არის **skip კავშირების** (residual connections) დამატება -- ResNet-ში შემოღებული ტექნიკა, რომელიც მქრქალ გრადიენტებს ებრძვის გრადიენტს identity shortcut-ის გავლით პირდაპირი გზის მიცემით.
 
### ძირითადი იდეა: Skip კავშირები
 
ჩვეულებრივ conv ბლოკში, backprop-ის დროს გრადიენტი თანმიმდევრულად გაივლის ყველა conv-სა და არახაზობრიობას. ღრმა ქსელებში ეს მქრქალ გრადიენტებს იწვევს. Residual ბლოკი shortcut-ს ამატებს:
 
```
output = relu(f(x) + x)
```
 
სადაც `f(x)` ორი conv-ის გზაა და `x` არის identity. თუ ბლოკი კონკრეტული შეყვანისთვის სასარგებლო არ არის, `f(x)` ნულთან ახლოს მნიშვნელობების სწავლას შეძლებს და ბლოკი გახდება `relu(0 + x)` -- შეყვანას ხელუხლებლად გაატარებს. გრადიენტს ორი უკუ გზა აქვს: `f(x)`-ის გავლით (რომელმაც შეიძლება შეასუსტოს) და პირდაპირ identity-ის გავლით (რომელიც უცვლელად გადასცემს).
 
### 1x1 Conv პროექცია
 
როცა ბლოკებს შორის არხების რაოდენობა იცვლება (მაგ. 64->128), `x`-ს და `f(x)`-ს განსხვავებული ფორმა აქვს და პირდაპირ ვერ ემატება. shortcut გზაზე **1x1 conv** წყვეტს ამ პრობლემას -- `x`-ს საჭირო არხების რაოდენობამდე პროექტირებს სივრცული ცვლილების გარეშე, ყოველ სივრცულ პოზიციაზე დამოუკიდებლად გამოყენებული სწავლებული წრფივი პროექციის სახით:
...
მთავარი თანმიმდევრობა: `conv_b`-ს დამატებამდე ReLU არ გამოუყენებია. ReLU-ს დამატებამდე გამოყენება ორივე წევრს "გაჭრიდა", gradient-ის გავრცელებას შეზღუდავდა. `relu(f(x) + identity)` გრადიენტს identity გზით თავისუფლად ავრცელებს.
 
Pooling ხდება ResBlock-ს **გარეთ** `Net.forward`-ში, დამატების შემდეგ.
 
4-ბლოკიანი სტრუქტურა VGG4StyleNet-ს ასახავს პირდაპირი შედარებისთვის:
...
| გაშვება | შედულ. | აუგ. | ეპოქები | Train | Val | შენიშვნა |
|---|---|---|---|---|---|---|
| R1 | არა | არა | 80 | -- | -- | *(შემავსებელია)* |
| R2 | RLP (0.7/7) | არა | 100 | -- | -- | *(შემავსებელია)* |
| R3 | RLP (0.7/7) | Flip+Rot+Trans | 120 | -- | -- | *(შემავსებელია)* |
| R4 | RLP (0.7/7) | Flip+Rot+Trans+Jitter+Erasing | 120 | -- | -- | *(შემავსებელია)* |
 
---
 
## შემაჯამებელი ცხრილი — ყველა გაშვება
 
| გაშვება | მოდელი | LR | შედ. | აუგ. | ეპ. | Train | Val | შენიშვნა |
|---|---|---|---|---|---|---|---|---|
| 1 | 3-conv, **ნორმ. გარეშე** | 1e-3 | -- | -- | 50 | ~14% | ~14% | **გაფუჭებული** |
| 2 | 3-conv, ნორმ. | 1e-3 | -- | -- | 50 | up | up | სანიტი ჩეკი გაიარა |
...
| 5 | + შედ.(0.5/5) | 1e-3 | RLP | -- | 80 | ~65% | ~50% | შედ. ძალიან აგრ., LR ჩამოვარდა |
| 6 | + შედ.(0.7/7) + აუგ | 1e-3 | RLP | Flip+Rot+Trans | 80 | ~50% | ~57-60% | ოვერფიტი არ არის |
| 7 | **CustomCNN4** (32-64-128-256) | 1e-3 | RLP | GPU | 150 | ~70% | **~65%** | CustomCNN ჭერი |
| V1 | VGG3StyleNet | 1e-3 | RLP(0.5/5) | Flip+Rot+Trans | 100 | ~74% | ~67% | შედ. ხმაურიანი, სხვ. 7% |
| V2 | VGG3StyleNet | შეც. | RLP(0.7/7) | Flip+Rot+Trans | 150 | ~79% | ~68% | **დაბინძ.** -- V1 წონები |
| V3 | VGG3StyleNet | 3e-3 | RLP | Flip+Rot+Trans | ~35 | ~32% | ხმ. | LR ძალიან მაღ. |
| V4 | **VGG4StyleNet** | 1e-3 | RLP(0.5/5) | +ColorJitter | 100 | ~73% | **~70%** | **საუკეთესო, სხვ. 3%** |
| V5 | VGG4StyleNet | 1e-3 | RLP(0.7/7) | Flip+Rot+Trans | 120 | ~70% | ~60% | გადარეგ. |
| R1 | ResNet-style | 1e-3 | -- | -- | 80 | -- | -- | *(შემავსებელია)* |
| R2 | ResNet-style | 1e-3 | RLP(0.7/7) | -- | 100 | -- | -- | *(შემავსებელია)* |
| R3 | ResNet-style | 1e-3 | RLP(0.7/7) | Flip+Rot+Trans | 120 | -- | -- | *(შემავსებელია)* |
| R4 | ResNet-style | 1e-3 | RLP(0.7/7) | +Jitter+Eras. | 120 | -- | -- | *(შემავსებელია)* |
...
2. **სანიტი ჩეკი (`loss@init` + 4 ნიმუშზე ოვერფიტი) სანდოდ განასხვავებს "პაიფლაინის ბაგს" "მეტი ტრენინგი სჭირდება"-სგან.** ამ პროექტის ყველა არქიტექტურული ცვლილება სანიტი ჩეკით შემოწმდა.
...
4. **LR შედულერის აგრესიულობა დიდ მნიშვნელობას ატარებს.** `factor=0.5, patience=5` LR-ს 64-ჯერ ამცირებს ~40 ეპოქაში. `factor=0.7, patience=7, min_lr=1e-5` უფრო რბილია და LR-ს სასარგებლო ზონაში ინარჩუნებს.
5. **"ჯერ ოვერფიტი, შემდეგ რეგულარიზაცია."** გაშვება 6 (აუგმენტირებული, 3-conv) 80-ე ეპოქაზე პლატოს ვერ მიაღწია -- სიგნალი 4-ე conv ბლოკის დამატებისთვის, არა მეტი რეგულარიზაციისთვის.
6. **ნუ გამოასწორებ ისეთ რასაც გასწორება არ სჭირდება.** VGG4 გაშვება 1-ს 3%-იანი train/val სხვაობა ჰქონდა. დამატებითი რეგულარიზაცია მოდელი underfitting-ში ჩააგდო.
7. **VGG-სტილის double-conv ბლოკები single-conv-ებს მუდმივად ჯობნის.** CustomCNN4 (65%)-დან VGG3StyleNet (67%)-ზე გადასვლა მხოლოდ ბლოკის სტრუქტურის შეცვლით მოხდა.
8. **ყოველ გაშვებამდე model, optimizer და scheduler ხელახლა ინიციალიზება.** VGG გაშვება 2 V1-ის წონებიდან გაგრძელდა -- გაფართოებული სხვაობა სტალე წონების მდგომარეობით გამოიწვია, არა შედულერის ცვლილებით.
9. **Skip კავშირები გრადიენტს პირდაპირ გზას აძლევს.** მოდელს შეუძლია ბლოკი "გამოტოვოს" თუ ის სასარგებლო არ არის. 1x1 conv shortcut კი სხვადასხვა ზომის ბლოკებს შორის არხების განზომილებების შეჯერებას უზრუნველყოფს.